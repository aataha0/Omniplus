...
export class LeaGradesOverview extends Renderable<null> {
    ...
    static loadFromGradesOverviewPage(page: Document) {
        ...
        return new LeaGradesOverview(Promise.all(Array.from<HTMLAnchorElement>(page.querySelectorAll('.tableau-notes td:nth-child(3) a'))
            // Extract the link to the class's page and load them.
            .map((anchor) => fetchDocumentFrom(anchor.href)))
